...
export default function Navbar() {
  return (
    <>
        {/* Mobile app navbar */}
        <nav className="fixed inset-x-0 bottom-0 z-50 border-t 
                      border-slate-200 dark:border-slate-700
                      bg-white dark:bg-slate-800 md:hidden">
                    ...
                    <li key={href} className="flex-1">
                        <Link
                           ...
                           text-zinc-500 dark:text-slate-400">
                                <Icon className="text-base text-zinc-900 dark:text-slate-50" />
                                <span>{label}</span>
                        </Link>
                    </li>
                ))}
        ...
        {/* Desktop web navbar */}
        <nav className="hidden border-b 
                      border-slate-200 dark:border-slate-700
                      bg-white dark:bg-slate-800 md:block">
                    ...
                    {navItems.map(({ label, href, icon: Icon }) => (
                        <li key={href}>
                            ...
                            className="flex items-center gap-1 rounded-md px-3 py-2 text-sm 
                             text-zinc-500 dark:text-slate-400">
                                <Icon className="text-sm text-zinc-900 dark:text-slate-50" />
                                <span>{label}</span>
                            </Link>
                        </li>
                    ))}
